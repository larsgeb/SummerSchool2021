...
namespace kernels {
__global__ static void fill_device(double* ptr, double val, size_t n) {
...
class Field {
 public:
  // default constructor
  Field() : host_ptr_(nullptr), device_ptr_(nullptr), xdim_(0), ydim_(0){};

  // constructor
  Field(int xdim, int ydim)
      : host_ptr_(nullptr), device_ptr_(nullptr), xdim_(xdim), ydim_(ydim) {
    init(xdim, ydim);
  };
...
#pragma omp parallel for
    for (int i = 0; i < xdim_ * ydim_; ++i) host_ptr_[i] = val;
  }
